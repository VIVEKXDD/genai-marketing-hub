...
def generate_campaign(request: CampaignRequest):
    try:
        ...
        taglines = AIGeneration.generate_campaign_hashtags(request.campaign_name, request.product, request.target_audience, request.objective)
        themes = AIGeneration.generate_campaign_hashtags(request.campaign_name, request.product, request.target_audience, request.objective)
        caption = AIGeneration.generate_campaign_hashtags(request.campaign_name, request.product, request.target_audience, request.objective)
        # Save to database
        campaign_data = {
